...
function showQuiz() {
    ...
    if (qIdx >= questions.length) {
        quizArea.innerHTML = `
            <div class="game-result" style="text-align:center; padding: 20px;">
                <h3>HOÀN THÀNH BÀI KIỂM TRA!</h3>
                <p>Điểm số của bạn: <strong style="color:#2ecc71; font-size:1.5em;">${qScore}/${questions.length}</strong></p>
                <button onclick="resetQuiz()" class="btn-action" style="background:#2ecc71; color:white; border:none; padding:10px 20px; border-radius:20px; cursor:pointer;">Làm lại</button>
            </div>`;
        return;
    }
    ...
    q.a.forEach((opt, i) => {
        ...
        btn.innerText = opt;
        btn.className = "quiz-btn"; // Hãy đảm bảo bạn có class này trong CSS
...
// Hàm xử lý kéo thả (Gắn vào ondragover và ondrop của HTML)
...
function initMemory() {
    ...
    shuffledIcons.forEach(icon => {
        ...
        card.innerHTML = `<span class="icon-hidden" style="visibility:hidden">${icon}</span>`;
